...
static func window(open: Tracked): return func(c: Ctx):
    ...
    c.on("tree_entered", func(): 
        var vis = c.node().get_parent().get_viewport().get_visible_rect()
        var t = (Vector2(vis.size) / 2) - (Vector2(c.node().min_size) / 2)
        c.node().popup_on_parent(Rect2i(t, Vector2())))
